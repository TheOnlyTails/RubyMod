...
public class RubyBarrelContainer extends Container {
    public final TileRubyBarrel tileEntity;
        ...
        for (int row = 0; row < 5; row++) {
            for (int column = 0; column < 9; column++) {
                this.addSlot(new SlotItemHandler(
                        tileEntity.itemHandler, (row * 9) + column,
        ...
        for (int row = 0; row < 3; row++) {
            for (int column = 0; column < 9; column++) {
                this.addSlot(new Slot(playerInventory, 9 + (row * 9) + column,
                        startX + (column * slotSizePlus2), playerInvStartY + (row * slotSizePlus2)));
            }
        }
    ...
    private static TileRubyBarrel getTileEntity(final PlayerInventory playerInventory,
                                                final PacketBuffer data) {
        Objects.requireNonNull(playerInventory, "playerInventory cannot be null");
        Objects.requireNonNull(data, "data cannot be null");
        final TileEntity tileAtPos = playerInventory.player.world.getTileEntity(data.readBlockPos());
        if (tileAtPos instanceof TileRubyBarrel) {
            return (TileRubyBarrel) tileAtPos;
        }
